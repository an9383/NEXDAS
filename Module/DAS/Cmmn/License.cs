	...
	public class License
	{
		...
		public string LicenseCheckProcess()
		{
			...
			try
			{
				...
				else
				{
					...
					else if (CModule.ToString(dtHederCheck.Rows[0]["USEFLAG"]) == "Y")
					{
						if (LicCloseDayCheck())
						{
		...
		private DataTable LicHederCheck()
		{
			...
			try
			{
				...
				rtnDtTemp = dBHelper.FillTable(CModule.ToString(stringBuilder));
				return rtnDtTemp;
			}
			catch
			{
		...
		private bool LicHederADD()
		{
			...
			try
			{
				...
				dBHelper.ExecuteNoneQuery(CModule.ToString(stringBuilder), CommandType.Text);
				dBHelper.Commit();
				return true;
			}
			catch
		...
		private bool LicDetailADD()
		{
			...
			try
			{
				string str = CModule.ToString(Dns.GetHostName());
				string text = CModule.ToString(NetworkInterface.GetAllNetworkInterfaces()[0].GetPhysicalAddress());
				if (text.Length != 12)
				{
					_msg.Exe_MessageForm(Common.getLangText("[E] 랜카드를 확인 하세요.", "MSG"), MessageBoxButtons.OK, "");
					_msg.ShowDialog();
					return false;
				}
				...
				{
					_msg.Exe_MessageForm(Common.getLangText("[E] IP 정보 수집을 실패 하였습니다.", "MSG"), MessageBoxButtons.OK, "");
					_msg.ShowDialog();
					return false;
				}
				stringBuilder = new StringBuilder();
		...
		private string LicDetailCheck()
		{
			...
			try
			{
				...
				{
					if (CModule.ToString(rtnDtTemp.Rows[0]["USEFLAG"]) == "N")
					{
		...
		private string LicOfflineCheck()
		{
			try
			{
				...
				RegistryKey registryKey2 = Registry.LocalMachine.OpenSubKey("Software\\WizLic").OpenSubKey("01");
				if (registryKey2 == null)
				{
					registryKey2 = Registry.LocalMachine.CreateSubKey("Software\\WizLic").CreateSubKey("01");
				...
				registryKey2 = Registry.LocalMachine.OpenSubKey("Software\\WizLic").OpenSubKey("01", writable: true);
				string text2 = Common.DecryptString(CModule.ToString(registryKey2.GetValue("ProdVer", string.Empty)));
				string text3 = Common.DecryptString(CModule.ToString(registryKey2.GetValue("DateKey", string.Empty)));
				string a = CModule.ToString(registryKey2.GetValue("ProdKey", string.Empty));
				registryKey2.Close();
