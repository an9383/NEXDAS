...
namespace Cmmn
{
	public class DBHelper
	{
		...
		public string ConType => _sConn.GetType().Name;

		public DbConnection DBConnect => _sConn;

		public DbParameterCollection Parameters => oCmd.Parameters;
		...
		private bool Init(string conStr, bool btran, bool completedClose)
		{
			try
			{
				if (conStr == "")
				{
                    conStr = Cmmn.CModule.GetAppSetting(Cmmn.CModule.sConnectionString, Cmmn.CModule.sDefualtConnectString);
				}
				if (Cmmn.CModule.GetAppSetting("ENCRYTION", "YES") == "YES")
				{
					conStr = Common.DecryptString(conStr);
				}
				_completedClose = completedClose;
				dataFactory = DbProviderFactories.GetFactory(Cmmn.CModule.GetAppSetting(Cmmn.CModule.sProvider, Cmmn.CModule.sDefualtProvider));
				_completedClose = completedClose;
		...
		public Database CreateDataBase()
		{
			...
			EnterpriseLibraryContainer.set_Current(EnterpriseLibraryContainer.CreateDefaultContainer((IConfigurationSource)(object)val2));
			return DatabaseFactory.CreateDatabase("DBConn");
		}

        */

		public void FillSchema(DataTable dt, string query, CommandType commandType)
		{
		...
		public void FillTable(DataTable dt, string query, CommandType commandType, bool baddLang, bool baddOut, params object[] parameters)
		{
			...
			if (commandType == CommandType.StoredProcedure)
			{
				if (baddLang)
				{
					oCmd.Parameters.Add(CreateParameter("Lang", DbType.String, Common.gsLanguege));
				}
				if (baddOut)
				{
					oCmd.Parameters.Add(CreateParameter("RS_CODE", DbType.String, ParameterDirection.Output, null, 1));
			...
			{
				RSMSG = CModule.ToString(oCmd.Parameters["RS_MSG"].Value);
				RSCODE = CModule.ToString(oCmd.Parameters["RS_CODE"].Value);
			}
			if (_completedClose)
			{
				Close();
		...
		public DataSet FillDataSet(string query, CommandType commandType, bool baddLang, bool baddOut, params object[] parameters)
		{
			...
			if (commandType == CommandType.StoredProcedure)
			{
				...
				{
					oCmd.Parameters.Add(CreateParameter("RS_CODE", DbType.String, ParameterDirection.Output, null, 1));
					oCmd.Parameters.Add(CreateParameter("RS_MSG", DbType.String, ParameterDirection.Output, null, 255));
				}
            }
            if (_sTran != null && oCmd.GetType().Name != "OracleCommand")
            {
		...
		public int ExecuteNoneQuery(string query, CommandType commandType, bool baddLang, bool baddOut, params object[] parameters)
		{
			...
			if (commandType == CommandType.StoredProcedure)
			{
				...
				if (baddOut)
				{
					oCmd.Parameters.Add(CreateParameter("RS_CODE", DbType.String, ParameterDirection.Output, null, 2));
					oCmd.Parameters.Add(CreateParameter("RS_MSG", DbType.String, ParameterDirection.Output, null, 200));
				}
			}
			if (_sTran != null && oCmd.GetType().Name != "OracleCommand")
		...
		public int ExecuteNoneQuery(string query, CommandType commandType, ref string RS_CODE, ref string RS_MSG, ref string TakingORDNO, params object[] parameters)
		{
			...
			int result = oCmd.ExecuteNonQuery();
			RS_MSG = CModule.ToString(oCmd.Parameters["RS_MSG"].Value);
			RS_CODE = CModule.ToString(oCmd.Parameters["RS_CODE"].Value);
			TakingORDNO = CModule.ToString(oCmd.Parameters["TakingORDNO"].Value);
			RSMSG = CModule.ToString(oCmd.Parameters["RS_MSG"].Value);
			RSCODE = CModule.ToString(oCmd.Parameters["RS_CODE"].Value);
			if (_completedClose)
			{
        ...
        public static int nvlInt(object obj, int def = 0)
		{
            ...
            if (isNumeric(obj))
            {
                if (nvlString(obj).Contains(","))
                {
                    obj = nvlDouble(nvlString(obj).Replace(",", ""));
                }
                obj = nvlDouble(obj);
