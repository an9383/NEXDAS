...
namespace NEXDAS
{
    public partial class DX9000 : BaseForm
    {
        #region [ MEMBER AREA ]
        private FormInfor FormInformation;

        private string sSearchOption;
        #endregion
        ...
        #region [ FORM EVENT ]
        private void DX9000_Shown(object sender, EventArgs e)
        {
            CheckRecDate();
        ...
        {
            DataRow dr = subData["METHOD_TYPE", "SUM_TYPE"];

            if (dr != null)
            {
                if (CModule.ToString(dr["RELCODE1"]) == "DANAWA")
                {
                    sSearchOption = "B";
                }
            }
            ...
            try
            {
                ...
                switch (CModule.ToString(sender.Tag))
                {
                    ...
                    case "SearchB":
                        ...
                        if (btnConfirm[0, 1].Text == "개별")
                        {
                            btnConfirm[0, 1].Text = "집계";
                        }
                        else
                        {
                            btnConfirm[0, 1].Text = "개별";
                        }
        ...
        {
            this.lblTitle.Text = Common.getLangText("생산실적 이력", "DAS");
            ...
            SetMessage(Common.getLangText("생산실적 이력 화면 입니다.", "DAS"));
        }
        ...
        private void DoFind()
        {
            switch ( CModule.ToString(btnConfirm[0, 1].Text))
            {
                case "":
                case "개별":
                    Grid1.SelectProcedureName = "USP_DX9000_S1";
                    break;
                case "집계":
                    Grid1.SelectProcedureName = "USP_DX9000_S2";
                    break;
            }
            ...
            if (Grid1.SelectProcedureName == "USP_DX9000_S2")
            {
                SetMessage(Grid1.RSMSG);
            }
            else
            {
