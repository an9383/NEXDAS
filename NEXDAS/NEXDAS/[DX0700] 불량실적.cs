...
namespace NEXDAS
{
    public partial class DX0700 : BaseForm
    {
        #region [ MEMBER AREA ]
        ...
        #region [ FORM EVENT ]
        private void DX0700_Shown(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (sLotNo != "")
                {
                    ...
                    for (int i = 0; i < Grid1.Rows.Count; i++)
                    {
                        ...
                        {
                            helper.ExecuteNoneQuery("USP_DX0700_I2", CommandType.StoredProcedure
                            , helper.CreateParameter("AS_PLANTCODE", Common.SelectedWorkCenter.PlantCode, DbType.String, ParameterDirection.Input)
                ...
                else
                {
                    if (iSeqNO < 0)
                    {
                        helper.ExecuteNoneQuery("USP_DX0700_I1", CommandType.StoredProcedure
                        , helper.CreateParameter("AS_PLANTCODE", Common.SelectedWorkCenter.PlantCode, DbType.String, ParameterDirection.Input)
                        , helper.CreateParameter("AS_WORKCENTERCODE", CModule.ToString(lblWC.Tag), DbType.String, ParameterDirection.Input)
                    ...
                    else
                    {
                        ...
                        helper.ExecuteNoneQuery("USP_DX0700_U1", CommandType.StoredProcedure
                ...
                if (sLotNo != "")
                {
                    if (MessageBoxShow(Common.getLangText("선택한 불량 항목을 삭제 처리하시겠습니까?", "DAS"), MessageBoxButtons.YesNo) == DialogResult.Yes)
                    {
                        ...
                        for (int i = 0; i < Grid2.Rows.Count; i++)
                        {
                            ...
                            if (sSel != "")
                            {
                                helper.ExecuteNoneQuery("USP_DX0700_D2", CommandType.StoredProcedure
                                , helper.CreateParameter("AS_PLANTCODE", Common.SelectedWorkCenter.PlantCode, DbType.String, ParameterDirection.Input)
