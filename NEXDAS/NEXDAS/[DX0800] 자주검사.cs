...
namespace NEXDAS
{
    public partial class DX0800 : BaseForm
    {
        #region [ MEMBER AREA ]
        private SerialPort[] _srp;

        private string sPort_COM11 = string.Empty;
        private string sPort_COM12 = string.Empty;
        private string sPort_COM13 = string.Empty;
        private string sPort_COM14 = string.Empty;
        ...
        #region [ FORM EVENT ]
        private void DX0800_Shown(object sender, EventArgs e)
        {
            this.Refresh();
            ...
            // inho.hwang 20.11.18
            // 작업조건 추가
            DataSet dsChk = helper.FillDataSet("USP_DX0800_S5", CommandType.StoredProcedure
            ...
            // 자주검사 자동넘김 기능 
            dt = dsChk.Tables[0];
            DataRow[] dArr = dt.Select("MethodCode = 'MC0085'");
            // 자주검사 자동넘김 기능을 사용할때 / 작업조건이 들어가 있을때 bMoveded true 처리
        ...
        /// <summary>
        /// 다음 항목으로 이동
        /// 2020-11-13 WSRYU 추가 작업 // 2020-11-18 inho.hwang DX0850 공정검사 참조
        /// </summary>
        ...
        private void serialPort_DataReceived(object sender, SerialDataReceivedEventArgs e)
        {
            ...
            switch (((SerialPort)(sender)).PortName)
            {
                case "COM11":
                    if (sData.IndexOf("\r") > 0)
                    {
                        sPort_COM11 = sPort_COM11 + sData;

                        if (sPort_COM11.Substring(0, 2) == "91")
                        {
                            SetMessage("11번 포트 연결을 확인 하세요.");
                            sPort_COM11 = string.Empty;
                        ...
                        for (int i = 0; i < Grid1.Rows.Count; i++)
                        {
                            ...
                            {
                                Grid1.Rows[i].Cells["RESULT"].Value = DBHelper.nvlDouble(sPort_COM11.Substring(3, 9));

                                Grid1_GridClick(Grid1, new zGrid.GridClickEventArg(Grid1.Rows[i].Cells[0]));
                                Grid1.Row = Grid1.Rows[i];
                                Grid1.Row.Selected = true;

                                if (CModule.ToString(Grid1.Row.Cells["RESULT"].Value) == "")
                                {
                                    return;
                                }
                                else
                                {
                                    if ((DBHelper.nvlDouble(Grid1.Row.Cells["SPECUSL"].Value) >= DBHelper.nvlDouble(Grid1.Row.Cells["RESULT"].Value)) && (DBHelper.nvlDouble(Grid1.Row.Cells["SPECLSL"].Value) <= DBHelper.nvlDouble(Grid1.Row.Cells["RESULT"].Value)))
                                    {
                ...
                case "COM12":
                    if (sData.IndexOf("\r") > 0)
                    {
                        sPort_COM12 = sPort_COM12 + sData;

                        if (sPort_COM12.Substring(0, 2) == "91")
                        {
                            SetMessage("12번 포트 연결을 확인 하세요.");
                            sPort_COM12 = string.Empty;
                        ...
                        for (int i = 0; i < Grid1.Rows.Count; i++)
                        {
                            if (((SerialPort)(sender)).PortName == CModule.ToString(Grid1.Rows[i].Cells["COMPORT"].Value))
                            {
                                Grid1.Rows[i].Cells["RESULT"].Value = DBHelper.nvlDouble(sPort_COM12.Substring(3, 9));
                                ...
                                else
                                {
                                    if ((DBHelper.nvlDouble(Grid1.Row.Cells["SPECUSL"].Value) >= DBHelper.nvlDouble(Grid1.Row.Cells["RESULT"].Value)) && (DBHelper.nvlDouble(Grid1.Row.Cells["SPECLSL"].Value) <= DBHelper.nvlDouble(Grid1.Row.Cells["RESULT"].Value)))
                                    {
                                        Grid1.Row.Cells["RESULT"].Appearance.BackColor = Color.ForestGreen;
                                        Grid1.Row.Cells["RESULT"].Appearance.ForeColor = Color.White;
                                        Grid1.Row.Cells["OKNG"].Value = "OK";
                                    }
                                    else
                                    {
                                        Grid1.Row.Cells["RESULT"].Appearance.BackColor = Color.Red;
                                        Grid1.Row.Cells["RESULT"].Appearance.ForeColor = Color.White;
                                        Grid1.Row.Cells["OKNG"].Value = "NG";
                                    }
                                }
                            }
                        }
                ...
                case "COM13":
                    if (sData.IndexOf("\r") > 0)
                    {
                        sPort_COM13 = sPort_COM13 + sData;

                        if (sPort_COM13.Substring(0, 2) == "91")
                        {
                            SetMessage("13번 포트 연결을 확인 하세요.");
                            sPort_COM13 = string.Empty;
                        ...
                        for (int i = 0; i < Grid1.Rows.Count; i++)
                        {
                            if (((SerialPort)(sender)).PortName == CModule.ToString(Grid1.Rows[i].Cells["COMPORT"].Value))
                            {
                                Grid1.Rows[i].Cells["RESULT"].Value = DBHelper.nvlDouble(sPort_COM13.Substring(3, 9));
                ...
                case "COM14":
                    if (sData.IndexOf("\r") > 0)
                    {
                        sPort_COM14 = sPort_COM14 + sData;

                        if (sPort_COM14.Substring(0, 2) == "91")
                        {
                            SetMessage("14번 포트 연결을 확인 하세요.");
                            sPort_COM14 = string.Empty;
                        ...
                        for (int i = 0; i < Grid1.Rows.Count; i++)
                        {
                            if (((SerialPort)(sender)).PortName == CModule.ToString(Grid1.Rows[i].Cells["COMPORT"].Value))
                            {
                                Grid1.Rows[i].Cells["RESULT"].Value = DBHelper.nvlDouble(sPort_COM14.Substring(3, 9));
        ...
        private void Grid1_GridKeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            if (sData_Temp.IndexOf("\r") >= 0)
            {
                ...
                Grid1.Row.Cells["RESULT"].Value = sData_USB.Trim();
                sData_USB = string.Empty;
        ...
        {
            this.lblTitle.Text = Common.getLangText("자주검사 등록", "DAS");
            ...
            lblInSP_T.Text     = Common.getLangText("검사 항목", "DAS");
            lblTitle01_T.Text  = "[ ① " + Common.getLangText("자주검사 이미지", "DAS") + " ]";
            lblTitle02_T.Text  = "※ " + Common.getLangText("최근 검사 일시") + " : " + Common.getLangText("최근 자주검사 이력 없음", "DAS");
            lblTitle03_T.Text  = "[ ② " + Common.getLangText("자주검사 리스트", "DAS") + " ]";
            ...
            btnExpand.BackgroundImage = (Image)Cmmn.Properties.Resources.ResourceManager.GetObject("DX0800_000");
			...
			tlpDX0800_01.BackColor = _clr;
			lblFormName.ForeColor  = _clr;

            lblFormName.Text = this.Name;

            SetMessage(Common.getLangText("자주검사를 실시 하세요.", "DAS"));
        }
        ...
        private void SetButton()
        {
            ...
            btnInspect_SEQ.SelectProcedureName = "USP_DX0800_S4";
        ...
        private void SetGrid()
        {
            ...
            Grid1.SelectProcedureName = "USP_DX0800_S1";
        }
        ...
        private void DoFind()
        {
            ...
            Grid1.ParmN = new string[] { "AS_PLANTCODE", "AS_ITEMCODE", "AS_WORKCENTERCODE","AS_TAG" };
            Grid1.ParmV = new string[] { Common.SelectedWorkCenter.PlantCode, CModule.ToString(lblItem.Tag), Common.SelectedWorkCenter.Code, sInspectSeq };
            Grid1.ParmT = new DbType[] { DbType.String, DbType.String , DbType.String, DbType.String };
            Grid1.DoFind();

            SetMessage(CModule.ToString(Grid1.Rows.Count) + " " + Common.getLangText("건이 조회 되었습니다.", "DAS") + " " + Common.getLangText("자주검사를 실시 하세요.", "DAS"));
            ...
            try
            {
                for (int i = 0; i < Grid1.Rows.Count; i++)
                {
                    if (CModule.ToString(Grid1.Rows[i].Cells["VALUE"].Value) != CModule.ToString(Grid1.Rows[i].Cells["PREVALUE"].Value))
                    {
                        ...
                        helper.ExecuteNoneQuery("USP_DX0800_I1", CommandType.StoredProcedure
        ...
        private void CheckImage(int idx)
        {
            ...
            try
            {
                DataTable dtImage = helper.FillTable("USP_DX0800_S2", CommandType.StoredProcedure
                                  , helper.CreateParameter("AS_PLANTCODE", Common.SelectedWorkCenter.PlantCode,                       DbType.String, ParameterDirection.Input)
                                  , helper.CreateParameter("AS_ITEMCODE",  CModule.ToString(lblItem.Tag),                             DbType.String, ParameterDirection.Input)
        ...
        private void OpenSerial()
        {
            ...
            try
            {
                DataTable dtSerial = helper.FillTable("USP_DX0800_SERIAL", CommandType.StoredProcedure
        ...
        private void GetLastDate()
        {
            ...
            try
            {
                DataTable dtLastDate = helper.FillTable("USP_DX0800_S3", CommandType.StoredProcedure
                                     , helper.CreateParameter("AS_PLANTCODE",      Common.SelectedWorkCenter.PlantCode, DbType.String, ParameterDirection.Input)
                                     , helper.CreateParameter("AS_WORKCENTERCODE", CModule.ToString(lblWC.Tag),         DbType.String, ParameterDirection.Input));
                ...
                lblTitle02_T.Text = sLastInSP == string.Empty ? "※ " + Common.getLangText("최근 검사 일시") + " : " + Common.getLangText("최근 자주검사 이력 없음", "DAS") : "※ " + Common.getLangText("최근 검사 일시") + " : " + sLastInSP;
            }
            catch (Exception ex)
            {
                SetMessage(ex.Message);
