...
namespace NEXDAS
{
    public partial class DX9040 : BaseForm
    {
		#region [ MEMBER AREA ]		
        ...
        #region [ FORM EVENT ]
        private void DX9040_Shown(object sender, EventArgs e)
        {
			SetButton();
            DoFind();
		...
		private void DX9040L_FormClosing(object sender, FormClosingEventArgs e)
		{
			if (_timer != null)
			{
				_timer.Stop();
		...
		private void Initialization()
        {
			...
			rtxNotice.BorderStyle  = BorderStyle.FixedSingle;
			...
			lblBG01.BackgroundImage = (Image)Cmmn.Properties.Resources.ResourceManager.GetObject("DX9040_000");

			btnConfirm.Location = new Point((lblBG01.Location.X - btnConfirm.Width), lblBG01.Location.Y);

			tlpDX9040.BackColor   = _clr01;
			lblFormName.ForeColor = _clr01;
			rtxNotice.BackColor   = _clr02;			
		...
		private void DoFind()
        {
            ...
            try
            {
				DataTable dtNotice = helper.FillTable("USP_DX9040_S1", CommandType.StoredProcedure
                                   , helper.CreateParameter("AS_PLANTCODE", Common.gsPlantCode, DbType.String, ParameterDirection.Input));

				if (dtNotice.Rows.Count > 0)
				{
					rtxNotice.Text = CModule.ToString(dtNotice.Rows[0]["MTTEXT"]);
					rtxNotice.Rtf  = CModule.ToString(dtNotice.Rows[0]["MTRTF"]);
				}
            }
            catch (Exception ex)
            {
